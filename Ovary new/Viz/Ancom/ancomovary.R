setwd("C:/Users/sanyu/OneDrive/Desktop/Ovary new/Viz/Ancom")
library(ggplot2)
library(ggrepel)
library(dplyr)

# Load the data (full ANCOM-BC2 table with all taxa)
# Expecting columns like:
# taxon, lfc_(Intercept), lfc_GroupTumor, ..., lfc, qval, diff, diff_robust,
# significant, direction, etc.
data <- read.csv("imp10pct.csv", header = TRUE)

# Use qval (for GroupTumor) as the adjusted p-value
padj_cutoff <- 0.05
...
data <- data %>%
  mutate(
    ...
    # Direction based on lfc sign + padj
    direction_label = case_when(
      padj < padj_cutoff & lfc > 0 ~ "Tumor_enriched",
      padj < padj_cutoff & lfc < 0 ~ "NAT_enriched",
...
plot_data <- data %>%
  filter(is.finite(lfc), is.finite(neglog10padj))
...
# Volcano plot
volcano_ancom <- ggplot(plot_data,
                        aes(x = lfc,
                            y = neglog10padj)) +
  ...
  labs(
    title = "ANCOM-BC2 Volcano Plot, Ovary Imputed: Tumor vs NAT (10% filter)",
    x     = "ANCOM-BC2 log2 fold change (Tumor − NAT)",
...
# Save plot
ggsave("vpimp10pct_ancom.png", plot = volcano_ancom,
       width = 10, height = 8, dpi = 300)
ggsave("vpimp10pct_ancom.pdf", plot = volcano_ancom,
       width = 10, height = 8)
