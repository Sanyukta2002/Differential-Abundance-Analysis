## =========================================================
## DA on mbImpute-imputed data (genus level)
## Design: paired NAT vs Tumor (Patient_ID)
## Prevalence filter: 5% (external only)
## Tools: DESeq2, ALDEx2, ANCOM-BC2, MaAsLin2
## Files:
##   - otu_imputed_origlibsize_genus_from_paired308.csv
##   - meta_mbimpute_genus_paired.csv
## =========================================================

setwd("C:/Users/sanyu/OneDrive/Desktop/Breast_new/Imputed_data")
...
prev_cut <- 0.05        # 5% prevalence
base_dir <- "differential_abundance_prev5_imputed"
...
counts_prev5 <- counts_paired[keep_taxa, , drop = FALSE]

cat("5% prevalence filter:",
    "keeping", sum(keep_taxa), "of", nrow(counts_paired), "taxa\n")

## =========================================================
## 1) DESeq2 (paired, 5% prev, imputed data)
## =========================================================
...
counts_deseq <- round(counts_prev5)
...
write_csv(
  res_deseq,
  file.path(out_dir_deseq, "DESeq2_prev5_imputed_Tumor_vs_NAT_all.csv")
)

write_csv(
  res_deseq %>% filter(significant),
  file.path(out_dir_deseq, "DESeq2_prev5_imputed_Tumor_vs_NAT_significant.csv")
)
...
write_csv(
  ...
  file.path(out_dir_deseq, "DESeq2_prev5_imputed_Tumor_vs_NAT_summary.csv")
)

## =========================================================
## 2) ALDEx2 (paired, 5% prev, imputed)
## =========================================================
...
counts_aldex <- counts_prev5[, meta_ord[[sample_id_col]], drop = FALSE]
...
cat("\nALDEx2 group counts (imputed, prev 5%):\n")
print(table(conds))
...
write_csv(
  res_aldex,
  file.path(out_dir_aldex, "ALDEx2_prev5_imputed_Tumor_vs_NAT_all.csv")
)

write_csv(
  res_aldex %>% filter(significant),
  file.path(out_dir_aldex, "ALDEx2_prev5_imputed_Tumor_vs_NAT_significant.csv")
)
...
write_csv(
  ...
  file.path(out_dir_aldex, "ALDEx2_prev5_imputed_Tumor_vs_NAT_summary.csv")
)

## =========================================================
## 3) ANCOM-BC2 (paired via random effect, 5% prev, imputed)
## =========================================================
...
cat("\nANCOM-BC2 (imputed): taxa =", nrow(counts_prev5),
    ", samples =", ncol(counts_prev5), "\n")
...
cat("\nANCOM-BC2 (imputed, prev 5%, paired)\n")
...
write_csv(
  res_ancom_tbl,
  file.path(out_dir_ancom, "ANCOMBC2_prev5_imputed_Tumor_vs_NAT_full.csv")
)

write_csv(
  res_ancom_tbl %>% filter(significant),
  file.path(out_dir_ancom, "ANCOMBC2_prev5_imputed_Tumor_vs_NAT_significant_q05.csv")
)

write_csv(
  res_ancom_tbl %>% filter(significant_robust),
  file.path(out_dir_ancom, "ANCOMBC2_prev5_imputed_Tumor_vs_NAT_significant_robust_q05.csv")
)
...
write_csv(
  ...
  file.path(out_dir_ancom, "ANCOMBC2_prev5_imputed_Tumor_vs_NAT_summary.csv")
)

## =========================================================
## 4) MaAsLin2 (paired via random effect, 5% prev, imputed)
## =========================================================
...
stopifnot(all(colnames(counts_prev5) == rownames(meta_maaslin)))

fit_maaslin <- Maaslin2(
  input_data      = counts_prev5,
...
cat("\nMaAsLin2 (imputed, prev 5%, paired)\n")
...
write_csv(
  res_group,
  file.path(out_dir_maaslin, "Maaslin2_prev5_imputed_Tumor_vs_NAT_all.csv")
)

write_csv(
  res_group %>% filter(significant),
  file.path(out_dir_maaslin, "Maaslin2_prev5_imputed_Tumor_vs_NAT_significant.csv")
)
...
write_csv(
  ...
  file.path(out_dir_maaslin, "Maaslin2_prev5_imputed_Tumor_vs_NAT_summary.csv")
)

cat("\nDONE: all four DA tools run on mbImpute-imputed data with 5% prevalence filter and paired design.\n")
