...
data <- read.csv("imp5_all.csv", header = TRUE)
...
volcano_plot <- ggplot(data, aes(x = log2FoldChange, y = -log10(padj))) +
  ...
  # Set axis labels
  labs(
    title = "Volcano Plot: Tumor vs Normal Adjacent Tissue, Imputed, 5pct filter",
...
ggsave("vp_imp5_all.png", plot = volcano_plot, width = 10, height = 8, dpi = 300)
ggsave("vp_imp5_all.pdf", plot = volcano_plot, width = 10, height = 8)
