setwd("C:/Users/sanyu/OneDrive/Desktop/Breast_new/Viz/ALDEX")
...
# In ALDEx we use we.eBH as the q val
padj_cutoff <- 0.05

# add padj and -ve log col and direction 
data <- data %>%
  mutate(
    # choose which adjusted p-value to use (here: Welch test BH)
    padj = we.eBH,
    ...
    direction_label = case_when(
      ...
      effect > 0                 ~ "Tumor_enriched",
      effect < 0                 ~ "NAT_enriched",
...
# keep only rows with finite x and y for plotting (safety)
plot_data <- data %>%
...
volcano_aldex <- ggplot(plot_data,
  ...
  labs(
    title = "ALDEx2 Volcano Plot: Tumor vs NAT (5% filter)",
    x     = "ALDEx2 effect (Tumor − NAT, CLR scale)",
...
# Save plot to files
ggsave("volcano_aldex.png", plot = volcano_aldex,
       width = 10, height = 8, dpi = 300)
ggsave("volcano_aldex.pdf", plot = volcano_aldex,
       width = 10, height = 8)
